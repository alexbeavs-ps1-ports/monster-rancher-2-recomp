...
#
# Writes: dist/monster-rancher-2-recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix monster-rancher-2-recomp \
  --exe-name Monster_Rancher_2_Recompiled \
  --display-name "Monster Rancher 2 Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Monster Rancher 2 disc" \
